...
with open("config_warmup_sft.yaml", "r") as f:
...
model, tokenizer = FastLanguageModel.from_pretrained(
    ...
    if config["optimization"].get("bf16", True)
    else None,
    load_in_4bit=False if need_merge else config["quantization"].get(
        "load_in_4bit",
        True,
    ),
...
dataset = load_dataset(
    ...
    data_files={
        ...
        "validation": config["dataset"]["val_path"],
    },
)
...
eval_dataset = dataset["validation"].filter(clean_filter)
...
training_args = SFTConfig(
    ...
    load_best_model_at_end=config["checkpoint"].get("load_best_model_at_end", True),
    metric_for_best_model="eval_loss",
    greater_is_better=False,
    lr_scheduler_type=config["scheduler"].get("type", "linear"),
    weight_decay=config["regularization"].get("weight_decay", 0.01),
    report_to="tensorboard",
    max_length=max_seq_length,
    dataset_text_field=text_column,
    packing=config["optimization"].get("packing", True), # 👈 Dynamically pulling packed status from config
    **warmup_kwargs,
)
...
try:
    ...
    print(f"Final merged model saved: {merged_path}")
